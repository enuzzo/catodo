...
- **Installation state and logo cache:** `installation-api.php` and `logo-cache.php` rely on the same signed HttpOnly gate cookie. Keep `.catodo-data/` non-public and writable only by the PHP account. The logo cache must retain HTTPS-only, public-IP, redirect, MIME and size checks; never broaden it into an arbitrary authenticated proxy.
