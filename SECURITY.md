...
- **Installation state and logo cache:** `installation-api.php` and `logo-cache.php` rely on the same signed HttpOnly gate cookie. Keep `.catodo-data/` non-public and writable only by the PHP account. Installation writes must remain versioned, whole-payload validated, revision-required, lock-serialized and atomically renamed; limits or malformed records must reject the write, and corrupt state must fail closed instead of becoming an empty installation. The server owns the legacy-migration marker: clients must not downgrade or replay it. The logo cache must retain HTTPS-only, public-IP, redirect, MIME and size checks; never broaden it into an arbitrary authenticated proxy.
