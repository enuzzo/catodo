...
export function filterTheatre(titles, { filter = 'all', collection = [], language = '', query = '', favorites = [], sort = 'editorial' } = {}) {
  ...
  return sortTheatreTitles(filtered, sort);
}

/** Sort a shelf copy; unknown dates/durations remain last in either direction. */
export function sortTheatreTitles(titles, order = 'editorial') {
  const sorted = [...titles];
  if (order === 'title') return sorted.sort((a, b) => a.title.localeCompare(b.title, 'en', { sensitivity: 'base', numeric: true }));
  const field = order === 'shortest' ? 'duration' : ['oldest', 'newest'].includes(order) ? 'year' : null;
  if (!field) return sorted;
  const direction = order === 'newest' ? -1 : 1;
  return sorted.sort((a, b) => {
    const validA = Number.isFinite(a[field]) && a[field] > 0;
    const validB = Number.isFinite(b[field]) && b[field] > 0;
    if (validA !== validB) return validA ? -1 : 1;
    return validA ? direction * (a[field] - b[field]) : 0;
  });
}
