# Historical roadmap and validation evidence

Archived on 2026-09-20 from the previous `docs/ROADMAP.md` without changing its
body. Release labels such as “Unreleased”, provider results and device status
below reflect earlier sessions, not current instructions or a fresh check.
Use [the active roadmap](../ROADMAP.md) for open work and
[the changelog](../../CHANGELOG.md) for release membership.
