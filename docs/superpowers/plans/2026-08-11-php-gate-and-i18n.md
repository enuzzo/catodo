...
**Interfaces:**
- Consumes: `app.html`, `index.php`, `.htaccess` from Task 4; `.htpasswd` already generated in the previous plan's Task 4 (or regenerate with `node gen-htpasswd.mjs` if it does not exist locally); `HTPASSWD_USER`/`HTPASSWD_PASSWORD` from `.env`.
...
Write and run a scratch Node script (same pattern used earlier in this session: read FTP credentials from `.env`, never print them) that uploads these five files with `basic-ftp`, replacing whatever `index.html` used to be live (delete the old `index.html` from the FTP root if it still exists there, since the new entry point is `index.php`).
...
3. `POST SITE_URL` with correct credentials, keeping the `Set-Cookie` from the response: response contains a marker unique to `app.html` (e.g. `CATODO` player specific text, or the string `buildGate`).
4. `GET SITE_URL` again reusing the session cookie: same authenticated content, no login form.
5. `GET SITE_URL + "/app.html"` directly: status `403`.
6. `GET SITE_URL + "/.htpasswd"` directly: status `403`.

Expected: all six checks pass. If step 5 or 6 does not return `403`, apply the same `AllowOverride` diagnosis used in the previous plan's Task 4 Step 5 (PHP probe for `DOCUMENT_ROOT`, but note this gate's `<Files>` blocking needs `AllowOverride` too, so if it was confirmed working for Basic Auth earlier, it should still work here unless the account structure changed).

- [ ] **Step 4: Report to the user**

Summarize the six results. No commit in this task, nothing in the repo changes.
